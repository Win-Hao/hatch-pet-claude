...
Usage: python3 extract.py <pet-dir>
"""
...
def remove_chroma(img, chroma_hex="#FF00FF", threshold=140.0):
    ...
    # Also remove near-white/gray backgrounds (canvas color from larger API output)
    gray_dist = np.sqrt((r-245)**2 + (g-245)**2 + (b-245)**2)
    pixels[(gray_dist < 30) & (a > 0)] = [0, 0, 0, 0]
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 extract.py <pet-dir>")
        sys.exit(1)

    pet_dir = Path(sys.argv[1])
    hatch_dir = pet_dir / ".hatch"

    if not (hatch_dir / "jobs.json").exists():
        print(f"ERROR: {hatch_dir / 'jobs.json'} not found. Run prepare.py first.")
        sys.exit(1)
    ...
    for job in jobs:
        if job["kind"] == "row-strip":
            ...
            row = job["row"]
            fc = job["frames"]
            strip_path = hatch_dir / job["output_path"]
    ...
    # Compose atlas → output to pet directory
    print("\n=== Composing atlas ===")
    ...
    atlas.save(pet_dir / "spritesheet.webp", quality=100, method=6)
    print("  Saved: spritesheet.png + spritesheet.webp")

    # Preview GIFs → pet directory
    print("\n=== Preview GIFs ===")
    gifs_dir = pet_dir / "previews"
